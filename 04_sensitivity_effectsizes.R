...
# Sensitivität / Imputation / Ausreißer + Effektstärken (ohne Plots)


source("R Skript/00_setup.R")
...
#3) ungeimputierter Summenscore (Regel: <=4 fehlend)
zbi_scored <- zbi_num |>
  dplyr::rowwise() |>
  dplyr::mutate(
    na_count = sum(is.na(dplyr::c_across(dplyr::all_of(zbi_items)))),
    zbi_total_raw = ifelse(
      na_count <= 4, sum(dplyr::c_across(dplyr::all_of(zbi_items)), na.rm = TRUE), NA_real_
    )
  ) |>
  dplyr::ungroup()

#4) Merge + Regression-Imputation (nur group)
...
#5) Zeitvariable + pre/post + Deduplizierung
...
# pro Patient & Zeitpunkt: den neuesten Eintrag
...
#6) Veränderungsliste (für klinische Prüfung)
...
# === Ausreißer (Top-Änderungen) identifizieren, anzeigen, speichern ===
#die größten absoluten Änderungen; bei Gleichstand (ties) alle
outlier_tbl <- zbi_changes_sorted %>%
...
# Konsolen-Output (gut lesbar)
cat("\n===== Top-Veränderungen (potenzielle Ausreißer) =====\n")
print(
  outlier_tbl %>%
...
# IDs für Sensitivitätsanalyse verwenden
outlier_ids <- unique(outlier_tbl$patient)

#7) ANOVA + Effektstärken
if (nrow(zbi_long_pairs) >= 3) {
  ...
  # Ausreißer: 2 stärkste Änderungen entfernen
  ...
  # Effektstärken
  # within-group dz = mean(post-pre)/sd(post-pre)
  ...
  # between-group (ΔI - ΔC): Hedges g (auf Change-Scores)
  ch <- zbi_long_pairs |>
  ...
  gt::gtsave(
    gt::gt(es_tbl) |>
      gt::tab_header(title = gt::md("**ZBI Effektstärken (pre→post)**"),
