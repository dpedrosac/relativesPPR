...
# 0) Pakete laden/ggf. installieren
pkgs <- c(
...
# 1) Pfad und Einlesen (robust bzgl. Encoding/NA/Typen)
# Pfade zu den aktuellen Daten (Exports vom 13.10.2025)
# Ordner mit den neuesten CSV-Dateien
...
read_latest <- function(filename) {
  readr::read_csv(
    ...
    locale       = readr::locale(encoding = "UTF-8"),  # notfalls "Latin1"
    na           = c("", "NA", "NaN", "NULL"),
    guess_max    = 100000,
...
# 2) Helfer: Patient*innen-ID robust erkennen
...
# 3) Datums-/Zeit-Parsing: universell & einheitlich POSIXct
#    - unterstützt ISO, d.m.Y[ H:M(:S)], m/d/Y, Excel-Seriennummern
...
parse_any_datetime <- function(x, tz = "UTC") {
  ...
  # Excel-Seriennummern (plausibler Bereich ~ 1997..2120)
  if (is.numeric(x)) {
  ...
  # Breiter Parser inkl. deutscher und ISO-Formate mit/ohne Uhrzeit
