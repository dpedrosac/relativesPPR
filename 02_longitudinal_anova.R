#!/usr/bin/env Rscript
# Script: 02_longitudinal_anova.R
# Purpose: prepare scales, run imputation and longitudinal ANOVAs (group × time)
# Version: 1.2
# Date: 2025-11-24
############################################################################################
# Setup / packages
...
source("00_setup.R")
need_extra <- c("afex", "ggplot2", "purrr") # additional dependencies
...
# 3) Prepare WHO-5 + imputation
# Note: 0 = "zu keinem Zeitpunkt"
...
# 5) CRF / Fatigue prepare + imputation
crf_item_names <- c(
...
# 6) Create pre/post long datasets + attach group
...
who5_long <- make_long_with_group(who5_imp, who5_pct,    "WHO-5 (0–100 %)")
bdi_long  <- make_long_with_group(bdi_imp,  bdi2_total,  "BDI-II (0–63)")
crf_long  <- make_long_with_group(crf_imp,  crf_total,   "CRF")
...
if (exists("sanity_check") && sanity_check == TRUE) {
  ...
  result_list <- purrr::map(long_list, summarise_score)
}

# 7) ANOVA per scale (group × time)
run_anova_summary <- function(dlong, instrument_label) {
  ...
  if (nrow(dplyr::filter(wide_tmp, !is.na(pre) & !is.na(post))) < 3) {
    return(NULL)  # too few pairs — skip this scale
  }
...
anova_gt <- anova_table |>
  ...
  gt::tab_header(
    ...
    subtitle = "Only participants with complete pre and post values"
  ) |>
  gt::fmt_missing(columns = dplyr::everything(), missing_text = "-")

# 8) Largest individual changes (ZBI/CRF)
zbi_pairs <- zbi_long |>
  dplyr::count(patient) |>
  dplyr::filter(n >= 2) |>
  dplyr::pull(patient)
...
# Note: check distributions before ANOVA; linear models may be more robust.